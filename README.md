...
- **No Telemetry & Tracking**: Zero background tracking or analytical telemetries are included. Full raw email body payloads are not uploaded; AI extraction uses selected, trimmed excerpts during user-approved scans.
...
- **SDK Packages**: `platforms;android-36`, `platforms;android-35` for JVM tests, `build-tools;36.0.0`, and `platform-tools`
- **JDK**: Version `17`
- **Gradle**: Use the checked-in Gradle wrapper (`./gradlew`), pinned to Gradle `9.3.1`
- **API Access**: A valid Google Gemini API Key

### 2. Gemini API Key Setup
Gemini API keys are configured at runtime in **Settings > Gemini API key** and stored only in encrypted device preferences. The release APK/AAB does not require or embed a build-time Gemini secret.

If encrypted storage is unavailable on the device, Gmail sync and Gemini extraction are disabled fail-closed.

### 3. Release Signing
Release builds are unsigned unless all signing environment variables are present. CI or a local release machine should provide:
...
1. **Plain, Accurate Disclosure**: To parse and auto-classify relevant applications, selected, trimmed email body excerpts are sent directly to the official Google Gemini API via HTTPS.
...
- Confirm CI is green for unit tests and release assembly.
